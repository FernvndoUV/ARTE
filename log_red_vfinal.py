...
def plot_folder(folder_name, log_per_img=1, cal_time=1, file_time=2,spect_time=1e-2,
    ...
    for i in range(n_img):
        ...
        data, avg_pow, clips, t, bases,flags = get_log_data(sublogs,cal_time, spect_time,
                file_time, decimation, mov_avg_size, tails, substract_median=substract_median)
        ...
        axes[0].axis(ymin =  np.median(avg_pow)-3 , ymax = np.median(avg_pow)+3)
       ...
       # graph = axes[1].pcolormesh(t,freq , data[:len(t),::].T, cmap = 'viridis',vmax = 290,vmin =  200,shading='auto' )
        graph = axes[1].pcolormesh(t,freq , ((data_new[:len(t),::])).T, cmap = 'viridis',vmax = std,vmin = -std,shading='auto' )
...
if __name__ == '__main__':
    if( not (len(sys.argv)>1)):
        ...
        plot_folder(
                ...
                substract_median=args.substract_median,
